...
export default function AdminDashboard() {
  ...
  const fetchUsers = async () => {
    ...
    } catch { showMsg("Gabim gjatë ngarkimit", "error"); }
    setLoading(false);
  };
  ...
  const saveUser = async () => {
    if (!userForm.name || !userForm.email || (!editUser && !userForm.password)) {
      showMsg("Plotëso të gjitha fushat!", "error"); return;
    }
    setUserLoading(true);
    try {
      let res;
      ...
      if (res.ok) {
        showMsg(editUser ? "Përdoruesi u përditësua!" : "Përdoruesi u shtua!");
        setUserModal(false);
        fetchUsers();
      } else {
  ...
  const deleteUser = async (id) => {
    ...
    else showMsg("Gabim gjatë fshirjes", "error");
  };
  ...
  const saveProp = async () => {
    if (!propForm.title || !propForm.price || !propForm.location) {
      showMsg("Plotëso fushat e detyrueshme!", "error"); return;
    }
    setPropLoading(true);
    try {
      let res;
      if (editProp) {
        res = await fetch(`${API}/admin/properties/${editProp.id}`, {
          method: "PUT", headers, body: JSON.stringify(propForm),
        });
      } else {
        res = await fetch(`${API}/properties`, {
          method: "POST", headers, body: JSON.stringify(propForm),
        });
      }
      const data = await res.json();
      if (res.ok) {
        showMsg(editProp ? "Prona u përditësua!" : "Prona u shtua!");
        setPropModal(false);
        fetchProperties();
      } else {
        showMsg(data.message || "Gabim", "error");
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f8fafc", fontFamily: "'Segoe UI', sans-serif" }}>
      ...
      {/* User Modal */}
      {userModal && (
        <Modal title={editUser ? "Ndrysho Përdoruesin" : "Shto Përdorues"} onClose={() => setUserModal(false)} onSave={saveUser} saving={userLoading}>
          ...
          <div><label style={labelStyle}>Fjalëkalimi {editUser ? "(lër bosh për të mos ndryshuar)" : "*"}</label><input style={inputStyle} type="password" placeholder="Min. 6 karaktere" value={userForm.password} onChange={(e) => setUserForm({ ...userForm, password: e.target.value })} /></div>
          <div>
      ...
      {/* Property Modal */}
      {propModal && (
        <Modal title={editProp ? "Ndrysho Pronën" : "Shto Pronë"} onClose={() => setPropModal(false)} onSave={saveProp} saving={propLoading}>
          <div><label style={labelStyle}>Titulli *</label><input style={inputStyle} placeholder="Apartament 2+1 në Prishtinë" value={propForm.title} onChange={(e) => setPropForm({ ...propForm, title: e.target.value })} /></div>
          <div><label style={labelStyle}>Përshkrimi</label><textarea style={{ ...inputStyle, height: 80, resize: "vertical" }} placeholder="Përshkrim i pronës..." value={propForm.description} onChange={(e) => setPropForm({ ...propForm, description: e.target.value })} /></div>
          <div><label style={labelStyle}>Çmimi (€) *</label><input style={inputStyle} type="number" placeholder="85000" value={propForm.price} onChange={(e) => setPropForm({ ...propForm, price: e.target.value })} /></div>
          <div><label style={labelStyle}>Lokacioni *</label><input style={inputStyle} placeholder="Prishtinë" value={propForm.location} onChange={(e) => setPropForm({ ...propForm, location: e.target.value })} /></div>
          <div><label style={labelStyle}>Tipi</label><input style={inputStyle} placeholder="Apartament / Shtëpi / Zyrë" value={propForm.type} onChange={(e) => setPropForm({ ...propForm, type: e.target.value })} /></div>
          <div>
      ...
      <div style={{ maxWidth: 1150, margin: "0 auto", padding: "32px 40px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 28 }}>
          ...
          <button onClick={tab === "users" ? openAddUser : openAddProp} style={{ padding: "10px 24px", background: "#2563eb", color: "white", border: "none", borderRadius: 10, fontWeight: 600, fontSize: 14, cursor: "pointer", boxShadow: "0 2px 8px rgba(37,99,235,0.3)" }}>
            {tab === "users" ? "Shto Perdorues" : "Shto Pron"}
          </button>
        </div>
        ...
        <div style={{ background: "white", borderRadius: 16, boxShadow: "0 1px 3px rgba(0,0,0,0.06), 0 4px 16px rgba(0,0,0,0.04)", overflow: "hidden", border: "1px solid #e2e8f0" }}>
          {loading ? (
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {users.length === 0 ? (
                  <tr><td colSpan={6} style={{ textAlign: "center", padding: "40px 0", color: "#94a3b8" }}>Nuk ka përdorues</td></tr>
                ) : users.map((u, i) => (
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ background: "#f8fafc", borderBottom: "2px solid #e2e8f0" }}>
                  {["ID", "Titulli", "Çmimi", "Lokacioni", "Tipi", "Statusi", "Veprime"].map((h) => (
                    <th key={h} style={{ padding: "14px 20px", textAlign: "left", fontSize: 11, fontWeight: 700, color: "#64748b", letterSpacing: "0.08em", textTransform: "uppercase" }}>{h}</th>
                  ))}
                </tr>
              </thead>
        ...
        <div style={{ marginTop: 16, color: "#94a3b8", fontSize: 13, textAlign: "right" }}>
          Gjithsej: <strong style={{ color: "#475569" }}>{tab === "users" ? users.length : properties.length}</strong> {tab === "users" ? "përdorues" : "prona"}
        </div>
      </div>
    </div>
  );
