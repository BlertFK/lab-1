...
const btnDanger = { background: "#fee2e2", color: "#dc2626", border: "1px solid #fca5a5", borderRadius: 8, padding: "6px 14px", cursor: "pointer", fontWeight: 600, fontSize: 13 };
const btnWarning = { background: "#fef3c7", color: "#92400e", border: "1px solid #fcd34d", borderRadius: 8, padding: "6px 14px", cursor: "pointer", fontWeight: 600, fontSize: 13, marginRight: 6 };
...
export default function AdminDashboard({ onLogout, setPage }) {
  ...
  const fetchUsers = async () => { setLoading(true); try { const res = await fetch(`${API}/admin/users`, { headers: getHeaders() }); setUsers(await res.json()); } catch { showMsg("Error loading users", "error"); } setLoading(false); };
  const fetchProperties = async () => { setLoading(true); try { const res = await fetch(`${API}/admin/properties`, { headers: getHeaders() }); setProperties(await res.json()); } catch { showMsg("Error loading properties", "error"); } setLoading(false); };

  useEffect(() => { if (tab === "users") fetchUsers(); else fetchProperties(); }, [tab]);

  const openAddUser = () => { setEditUser(null); setUserForm({ name: "", email: "", password: "", role: "buyer" }); setUserModal(true); };
  const openEditUser = (u) => { setEditUser(u); setUserForm({ name: u.name, email: u.email, password: "", role: u.role }); setUserModal(true); };

  const saveUser = async () => {
    if (!userForm.name || !userForm.email || (!editUser && !userForm.password)) { showMsg("Please fill in all fields!", "error"); return; }
    setUserLoading(true);
    try {
      let res;
      if (editUser) {
        ...
        res = await fetch(`${API}/admin/users/${editUser.id}`, { method: "PUT", headers: getHeaders(), body: JSON.stringify(body) });
      } else {
        res = await fetch(`${API}/auth/register`, { method: "POST", headers: getHeaders(), body: JSON.stringify(userForm) });
      }
      const data = await res.json();
      if (res.ok) { showMsg(editUser ? "User updated!" : "User added!"); setUserModal(false); fetchUsers(); }
  ...
  const openEditProp = (p) => { setEditProp(p); setPropForm({ title: p.title, description: p.description || "", price: p.price, location: p.location, type: p.type, status: p.status, image_url: p.image_url || "" }); setPropModal(true); };

  const saveProp = async () => {
    if (!propForm.title || !propForm.price || !propForm.location) { showMsg("Please fill in all required fields!", "error"); return; }
    setPropLoading(true);
    try {
      const res = await fetch(`${API}/admin/properties/${editProp.id}`, { method: "PUT", headers: getHeaders(), body: JSON.stringify(propForm) });
      const data = await res.json();
      if (res.ok) { showMsg("Property updated!"); setPropModal(false); fetchProperties(); }
      else showMsg(data.message || "Error", "error");
    } catch { showMsg("Error", "error"); }
  ...
  const roleBadge = (role) => { const c = { admin: ["#fef3c7","#92400e","#fcd34d"], seller: ["#dbeafe","#1e40af","#93c5fd"], buyer: ["#d1fae5","#065f46","#6ee7b7"] }; const [bg, color, border] = c[role] || c.buyer; return <span style={{ background: bg, color, border: `1px solid ${border}`, borderRadius: 999, padding: "2px 12px", fontSize: 12, fontWeight: 600 }}>{role}</span>; };
  const statusBadge = (status) => { const c = { available: ["#d1fae5","#065f46","#6ee7b7"], sold: ["#fee2e2","#991b1b","#fca5a5"], rented: ["#ede9fe","#5b21b6","#c4b5fd"] }; const [bg, color, border] = c[status] || c.available; return <span style={{ background: bg, color, border: `1px solid ${border}`, borderRadius: 999, padding: "2px 12px", fontSize: 12, fontWeight: 600 }}>{status}</span>; };
    ...
    <div style={{ minHeight: "100vh", background: "#f8fafc", fontFamily: "'Segoe UI', sans-serif" }}>
      <div style={{ background: "linear-gradient(135deg, #1e3a5f 0%, #2563eb 100%)", padding: "28px 40px", color: "white", boxShadow: "0 4px 20px rgba(37,99,235,0.3)" }}>
        <div style={{ maxWidth: 1150, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <div onClick={() => setPage("home")} style={{ fontSize: 20, fontWeight: 800, letterSpacing: "-0.01em", cursor: "pointer", marginBottom: 4 }}>UrbanKeys</div>
          ...
          <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
            ...
            <button onClick={onLogout} style={{ padding: "10px 20px", background: "rgba(255,255,255,0.15)", color: "white", border: "1px solid rgba(255,255,255,0.3)", borderRadius: 10, fontWeight: 600, fontSize: 14, cursor: "pointer" }}>Sign Out</button>
          </div>
        </div>
      </div>
      ...
      {userModal && (
        <Modal title={editUser ? "Edit User" : "Add User"} onClose={() => setUserModal(false)} onSave={saveUser} saving={userLoading}>
          <div><label style={labelStyle}>Name *</label><input style={inputStyle} placeholder="John Doe" value={userForm.name} onChange={(e) => setUserForm({ ...userForm, name: e.target.value })} /></div>
          <div><label style={labelStyle}>Email *</label><input style={inputStyle} type="email" placeholder="john@email.com" value={userForm.email} onChange={(e) => setUserForm({ ...userForm, email: e.target.value })} /></div>
          <div><label style={labelStyle}>Password {editUser ? "(leave blank to keep current)" : "*"}</label><input style={inputStyle} type="password" placeholder="Min. 6 characters" value={userForm.password} onChange={(e) => setUserForm({ ...userForm, password: e.target.value })} /></div>
          <div>
      ...
      {propModal && (
        <Modal title="Edit Property" onClose={() => setPropModal(false)} onSave={saveProp} saving={propLoading}>
          ...
          <div><label style={labelStyle}>Price ($) *</label><input style={inputStyle} type="number" placeholder="85000" value={propForm.price} onChange={(e) => setPropForm({ ...propForm, price: e.target.value })} /></div>
          <div><label style={labelStyle}>Location *</label><input style={inputStyle} placeholder="New York" value={propForm.location} onChange={(e) => setPropForm({ ...propForm, location: e.target.value })} /></div>
          <div><label style={labelStyle}>Type</label><input style={inputStyle} placeholder="Apartment / House / Villa" value={propForm.type} onChange={(e) => setPropForm({ ...propForm, type: e.target.value })} /></div>
          <div>
      ...
      <div style={{ maxWidth: 1150, margin: "0 auto", padding: "32px 40px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 28 }}>
          ...
          {tab === "users" && (
            <button onClick={openAddUser} style={{ padding: "10px 24px", background: "#2563eb", color: "white", border: "none", borderRadius: 10, fontWeight: 600, fontSize: 14, cursor: "pointer" }}>
              Add User
            </button>
          )}
        </div>

        <div style={{ background: "white", borderRadius: 16, overflow: "hidden", border: "1px solid #e2e8f0", boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
          {loading ? (
            <div style={{ textAlign: "center", padding: "60px 0", color: "#94a3b8", fontWeight: 500 }}>Loading...</div>
          ) : tab === "users" ? (
              ...
              <tbody>
                {users.length === 0 ? (
                  ...
                  >
                    ...
                    <td style={{ padding: "14px 20px", color: "#94a3b8", fontSize: 13 }}>{new Date(u.created_at).toLocaleDateString("en-US")}</td>
